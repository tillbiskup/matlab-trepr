...
% Matlab stinks! Therefore, we have to explicitly assign cell arrays AFTER
% we created the struct.
control.spectra.visible = {};
control.spectra.invisible = {};
control.spectra.modified = {};
control.spectra.missing = {};
...
if isempty(appdata.data{1})    % No data loaded to the GUI yet
    if iscell(spectra)         % Multiple spectra loaded
        for k=1:length(spectra)
            ...
            [path name ext] = fileparts(appdata.data{k}.filename);
            appdata.data{k}.label = name;
            clear path name ext
        end
        ...
        [path name ext] = fileparts(appdata.data{1}.filename);
        appdata.data{1}.label = name;
        clear path name ext
        iLoadedSpectra = [ 1 ];
    end
    ...
    if iscell(spectra)         % Multiple spectra loaded
        ...
        [path name ext] = fileparts(appdata.data{nSpectra+1}.filename);
        appdata.data{nSpectra+1}.label = name;
        clear path name ext
...
% --- Check for availability of necessary parameters in dataset
% If there are some parameters missing, the output variable TF is set to
% true.
function TF = if_checkForMissingParameters(hObject, id)
    ...
    case 'B0 spectra'
        ...
        for k = 1 : length(appdata.control.spectra.visible)
            ...
            if appdata.control.spectra.visible{k} == appdata.control.spectra.active && ...
                ~strcmp(appdata.control.axis.highlight.method,'none')
                plot(...
                    handles.axes1,...
                    ...
                    getfield(appdata.control.axis.highlight,'method'),...
                    getfield(appdata.control.axis.highlight,'value')...
                    );
            else
                plot(...
                    handles.axes1,...
...
if cp(1)>ac(1) && cp(1)<=ac(3) && cp(2)>ac(2) && cp(2)<=ac(4)
    ...
    if ~isempty(get(handles.axes1,'Children'))
        ...
        % first entry of the "xdata" cell array. Additionally, the order
        % of the xdata and ydata seems to be reversed
        if iscell(xdata)
            xdata = xdata{...
                length(xdata)-ind(appdata.control.spectra.visible,...
                appdata.control.spectra.active)+1};
            ydata = ydata{...
                length(ydata)-ind(appdata.control.spectra.visible,...
                appdata.control.spectra.active)+1};
        end

        % Handle zoomed situation (after user has zoomed in to the axes)
        % First, get current x and y limits of data displayed in the axes
        currentXLim = get(handles.axes1,'XLim');
        currentYLim = get(handles.axes1,'YLim');
        indXOffset = 1;
        indYOffset = 1;
        if xdata(1)<currentXLim(1) || xdata(end)>currentXLim(2) ...
            || ydata(1)<currentYLim(1) || ydata(end)>currentYLim(2)
            newXLim(1) = interp1(xdata,xdata,currentXLim(1),'nearest');
            if newXLim(1) > currentXLim(1)
                newXLim(1) = newXLim(1) - (xdata(2)-xdata(1));
            end
            newXLim(2) = interp1(xdata,xdata,currentXLim(2),'nearest');
            if newXLim(2) < currentXLim(2)
                newXLim(2) = newXLim(2) + (xdata(2)-xdata(1));
            end
            indXOffset = interp1(...
                xdata,1:length(xdata),newXLim(1),'nearest');
            xdata = xdata(...
                interp1(xdata,1:length(xdata),newXLim(1),'nearest'):...
                interp1(xdata,1:length(xdata),newXLim(2),'nearest')...
                );
            if strcmp(currentDisplayType,'2D plot')
                newYLim(1) = interp1(ydata,ydata,currentYLim(1),'nearest');
                if newYLim(1) > currentYLim(1)
                    newYLim(1) = newYLim(1) - (ydata(2)-ydata(1));
                end
                newYLim(2) = interp1(ydata,ydata,currentYLim(2),'nearest');
                if newYLim(2) < currentYLim(2)
                    newYLim(2) = newYLim(2) + (ydata(2)-ydata(1));
                end
                indYOffset = interp1(...
                    ydata,1:length(ydata),newYLim(1),'nearest');
                ydata = ydata(...
                    interp1(ydata,1:length(ydata),newYLim(1),'nearest'):...
                    interp1(ydata,1:length(ydata),newYLim(2),'nearest')...
                    );
            end
            indx=interp1(...
...
% --- Save spectrum in new file
function if_spectraSaveAs(hObject,varargin)
...
[pathstr, name] = fileparts(...
    appdata.data{iSelected}.filename);
[name, pathstr] = uiputfile('*.zip','Save current spectrum as',name);
if isempty(name)
    [name, pathstr] = uiputfile('*.zip');
elseif exist(fullfile(pathstr, [name '.zip']),'file')
    button = questdlg(...
        sprintf('%s\n%s\n%s',...
        'The file',...
    ...
    switch button
        case 'Yes'
        case 'No'
            [name, pathstr] = fileparts(uiputfile('*.zip'));
    end
end
trEPRsave(fullfile(pathstr,name),struct);
% Display dialog giving success feedback to the user
...
% Hints: contents = get(hObject,'String') returns fileTypePopupmenu contents as cell array
