...
%
%****f* file_handling.ascii_read_data/ascii_read_timeslice.m
%
% AUTHOR
...
%	$Revision: 422 $
% MODIFICATION DATE
%	$Date: 2007-09-27 13:41:51 +0100 (Do, 27 Sep 2007) $
% KEYWORDS
%	transient EPR, fsc2, ASCII read data
%
% SYNOPSIS
%	[ timeslice, timeaxis, field_params, time_params, frequency ] = ascii_read_timeslice ( filename )
%
% DESCRIPTION
%	This function reads the time slices that are saved with the function 
%	ascii_save_timeslice.m and returns the field and time parameters 
%	together with the data.
%
% INPUT PARAMETERS
%	filename
%		string containing the filename of the ASCII file containing the time slice
%		saved with the function ascii_save_timeslice that has to be read in
%
% OUTPUT PARAMETERS
%	timeslice
%		Nx1 vector containing the intensity values (in mV) of the time slice at the 
%		given field position.
%
%	timeaxis
%		Nx1 vector containing the values (normally in us) of the time axis
%
%	field_params
...
%
% EXAMPLE
%	To read the timeslice from the file 'filename' and get the timeslice (ts), the
%	time axis (ta), the field parameters (fp), time parameters (tp), and the
%	frequency (f), just typein:
%
%		[ ts, ta, fp, tp, f ] = ascii_read_timeslice ( filename )
%
% SOURCE

function [ timeslice, timeaxis, field_params, time_params, frequency ] = ascii_read_timeslice ( filename )

	fprintf('\nFUNCTION CALL: $Id: ascii_read_timeslice.m 422 2007-09-27 12:41:51Z till $\n\n');

	% check for the right number of input and output parameters

	if ( nargin ~= 1 )

		error('\n\tThe function is called with the wrong number (%i) of input arguments.\n\tPlease use "help ascii_read_timeslice" to get help.',nargin);

	end

	if ( nargout < 2 ) || ( nargout > 5 )

		error('\n\tThe function is called with the wrong number (%i) of output arguments.\n\tPlease use "help ascii_read_timeslice" to get help.',nargout);
	...
	if ( exist( filename )	== 2 )
		...
		timeslice=[];timeaxis=[];field_params=[];time_params=[];frequency=[];
	...
	while index (read, '%' ) == 1	% while read line starts with '%'
		...
		% "$ID: ascii_save_timeslice"
 	
		file_marker = findstr ( read, '$Id: ascii_save_timeslice.m' );
	...
	if ( is_right_file == 0 )
		...
		% display an error message and abort further operation
		%error('\n\tHmmm... %s does not look like a file\n\twritten with the function ascii_save_timeslice...\n\tAbort further operation...', filename);
	...
	raw_data = load ( filename );		% read data file (automatically ignore comments)

	timeaxis = raw_data(:,1);
	timeslice = raw_data(:,2);
