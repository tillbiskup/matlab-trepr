...
% 
%****f* file_handling.ascii_save_data/ascii_save_2Dspectrum.m
%
% AUTHOR
...
%
% SYNOPSIS
%	ascii_save_2Dspectrum ( filename, spectrum, field_params, time_params, [frequency] )
%
% DESCRIPTION
...
%
%	spectrum
%		a NxM matrix containing the data from the time slice
%
%	field_params
...
%	just typein:
%
%		ascii_save_2Dspectrum ( 'sp.dat', spectrum, fp, tp, freq )
%
% SOURCE

function ascii_save_2Dspectrum ( filename, spectrum, field_params, time_params, varargin )

	fprintf ( '\n%% FUNCTION CALL: $Id: ascii_save_2Dspectrum.m 422 2007-09-27 12:41:51Z till $\n%%' );
	
	% check for right number of input and output parameters

	if ( nargin < 4 ) || ( nargin > 5)
  
		error('\n\tThe function is called with the wrong number (%i) of input arguments.\n\tPlease use "help ascii_save_2Dspectrum" to get help.',nargin);
	...
	if nargout ~= 0
		...
		error('\n\tThe function is called with the wrong number (%i) of output arguments.\n\tPlease use "help ascii_save_2Dspectrum" to get help.',nargout);
			% get error if function is called with other than
	...
	elseif ( ~isnumeric(spectrum) || isvector(spectrum) || isscalar(spectrum) )

		error('\n\tThe function is called with the wrong format for the input argument %s.\n\tPlease use "help ascii_save_2Dspectrum" to get help.','spectrum');
			% get error if function is called with the wrong format of the
	...
	elseif ~isnumeric(field_params)
		...
		error('\n\tThe function is called with the wrong format for the input argument %s.\n\tPlease use "help ascii_save_2Dspectrum" to get help.','field_params');
			% get error if function is called with the wrong format of the
	...
	elseif ~isnumeric(time_params)
		...
		error('\n\tThe function is called with the wrong format for the input argument %s.\n\tPlease use "help ascii_save_2Dspectrum" to get help.','time_params');
			% get error if function is called with the wrong format of the
	...
	elseif nargin == 5
		...
		if ~isnumeric(frequency)
			...
			error('\n\tThe function is called with the wrong format for the input argument %s.\n\tPlease use "help ascii_save_2Dspectrum" to get help.','frequency');
				% get error if function is called with the wrong format of the
	...
	fprintf(fid, '%s This file has been written by the routine $Id: ascii_save_2Dspectrum.m 422 2007-09-27 12:41:51Z till $\n', comment_char);
	fprintf(fid, '%s\n', comment_char);
	...
	fprintf(fid, '%s\trows: time slices at given B_0 field position\n', comment_char);
	fprintf(fid, '%s\tcols: B_0 slices at given time position\n', comment_char);
	fprintf(fid, '%s\n', comment_char);
	...
	while (j <= rows)
		...
		fprintf(fid, '%+016.12f\t',spectrum(j,:));
		fprintf(fid, '\n');
		j = j + 1;
