...
%
%****f* file_handling.ascii_read_data/ascii_read_spectrum.m
%
% AUTHOR
...
%	$Revision: 422 $
% MODIFICATION DATE
%	$Date: 2006-06-28 17:03:00 +0000 (Wed, 28 Jun 2006) $
% KEYWORDS
%	transient EPR, fsc2, ASCII read data
%
% SYNOPSIS
%	[ spectrum, b0axis, field_params, time_params, frequency ] = ascii_read_spectrum ( filename )
%
% DESCRIPTION
%	This function reads the time slices that are saved with the function 
%	ascii_save_spectrum.m and returns the field and time parameters 
%	together with the data.
%
% INPUT PARAMETERS
%	filename
%		string containing the filename of the ASCII file containing the time slice
%		saved with the function ascii_save_spectrum that has to be read in
%
% OUTPUT PARAMETERS
%	spectrum
...
%	field axis (ba), the field parameters (fp), time parameters (tp), and the
%	frequency (f), just typein:
%
%		[ sp, ba, fp, tp, f ] = ascii_read_spectrum ( filename )
%
% SOURCE

function [ spectrum, b0axis, field_params, time_params, frequency ] = ascii_read_spectrum ( filename )

	fprintf('\n%% FUNCTION CALL: $Id: ascii_read_spectrum.m 262 2006-06-28 17:03:00 +0000 (Wed, 28 Jun 2006) web8 $\n');

	% check for the right number of input and output parameters

	if ( nargin ~= 1 )

		error('\n\tThe function is called with the wrong number (%i) of input arguments.\n\tPlease use "help ascii_read_spectrum" to get help.',nargin);

	end

	if ( nargout < 2 ) || ( nargout > 5 )

		error('\n\tThe function is called with the wrong number (%i) of output arguments.\n\tPlease use "help ascii_read_spectrum" to get help.',nargout);
	...
	if ( exist( filename )	== 2 )
		...
		spectrum=[];b0axis=[];field_params=[];time_params=[];frequency=[];
	...
	while index (read, '%' ) == 1	% while read line starts with '%'
		...
		% "$ID: ascii_save_spectrum"
 	
		file_marker = findstr ( read, '$Id: ascii_save_spectrum.m' );
	...
	field_params = [ min([start_field end_field]), max([start_field end_field]), abs(field_step_width) ];
