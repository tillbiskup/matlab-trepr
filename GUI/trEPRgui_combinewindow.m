...
for k=1:length(formatNames)
    if isfield(fileFormats.(formatNames{k}),'combineMultiple') && ...
        ...
        combinableFormats{end+1} = formatNames{k}; %#ok<AGROW>
    end
end
...
setappdata(hMainFigure,'combinableFormats',combinableFormats);
...
try
    ...
    for k=1:length(uniqueFormats)
        if max(strcmpi(uniqueFormats{k},ad.combinableFormats)) ...
...
try
    ...
    for k=1:length(uniqueFormats)
        if max(strcmp(uniqueFormats{k},ad.combinableFormats)) ...
                && length(find(strcmpi(uniqueFormats{k},formats)))>1
            displayFormats{end+1} = uniqueFormats{k}; %#ok<AGROW>
        end
    end
