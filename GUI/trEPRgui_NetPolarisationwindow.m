...
function updateAnalysisPanel()
    try
        ...
        NP = ad.data{active}.parameters.bridge.MWfrequency.value;
        if isscalar(NP) && length(ad.data{active}.parameters.bridge.calibration.values) > 1
            NP = ad.data{active}.parameters.bridge.calibration.values;
        end
        
        set(gh.mwfrequency_min_edit,'String',num2str(min(NP),'%10.6f'));
        set(gh.mwfrequency_max_edit,'String',num2str(max(NP),'%10.6f'));
        set(gh.mwfrequency_delta_edit,...
            'String',num2str(max(NP)-min(NP),7));
        set(gh.mwfrequency_avg_edit,'String',num2str(mean(NP),'%10.6f'));
        set(gh.mwfrequency_stdev_edit,'String',num2str(std(NP,1),'%10.6f'));
        ...
        fieldDeviationDelta = (max(NP)-min(NP))*1e9*hplanck /...
            (gelectron*bohrmagneton);
        fieldDeviationStdev = std(NP,1)*2*1e9*hplanck /...
            (gelectron*bohrmagneton);
    ...
    try
        ...
        if ad.control.axis.stdev
            patch(...
                ...
                'YData',...
                [mean(NP)-std(NP,1) mean(NP)-std(NP,1) ...
                mean(NP)+std(NP,1) mean(NP)+std(NP,1)],...
                'ZData',[0 0 0 0],...
