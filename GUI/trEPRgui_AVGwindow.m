...
uicontrol('Tag','average_pick_pushbutton',...
    'Style','pushbutton',...
    'Parent',pp2_p3,...
    'BackgroundColor',defaultBackground,...
    'Enable','on',...
...
ad.avg.area.size.start = 1;
ad.avg.area.size.stop = 1;
ad.avg.area.size.delta = 0;
    ...
    try
        ...
        switch position
            case 'startindex'
                ...
                if (value < 1) value = 1; end
                ad.avg.area.size.start = value;
                % Set stop value not overlapping
                if ad.avg.area.size.start > ad.avg.area.size.stop
                    % Check whether start + delta > length(dim)
                    if (ad.avg.area.size.start + ad.avg.area.size.delta > length(dim))
                        ad.avg.area.size.stop = length(dim);
                        ad.avg.area.size.delta = ...
                            ad.avg.area.size.stop-ad.avg.area.size.start;
                    else
                        ad.avg.area.size.stop = ...
                            ad.avg.area.size.start+ad.avg.area.size.delta;
                    end
                end
                % Set delta value
                ad.avg.area.size.delta = ...
            ...
            case 'stopindex'
                ...
                if (value > length(dim)) value = length(dim); end
                if (value < 1) value = 1; end
                ad.avg.area.size.stop = value;
                % Set start value not overlapping
                if ad.avg.area.size.start > ad.avg.area.size.stop
                    % Check whether stop - delta < length(dim)
                    if (ad.avg.area.size.stop - ad.avg.area.size.delta < 1)
                        ad.avg.area.size.start = 1;
                        ad.avg.area.size.delta = ...
                            ad.avg.area.size.stop-ad.avg.area.size.start;
                    else
                        ad.avg.area.size.start = ...
                            ad.avg.area.size.stop-ad.avg.area.size.delta;
                    end
                end
                % Set delta value
                ad.avg.area.size.delta = ...
            ...
            case 'deltaunit'
                ...
                if isnan(ad.avg.area.size.delta)
                    ad.avg.area.size.delta = 0;
                end
                % Check whether start + delta > length(dim)
                if (ad.avg.area.size.start + ad.avg.area.size.delta > length(dim))
...
function popupmenu_Callback(source,~,action)
    try
        ...
        switch action
            case 'dimension'
                dimensions = cellstr(get(source,'String'));
                dimension = dimensions{get(source,'Value')};
                ad.avg.dimension = dimension(1);
                setappdata(mainWindow,'avg',ad.avg);
    ...
    try
        ...
        if strcmpi(ad.avg.dimension,'x')
            set(gh.average_start_unit_edit,...
                'string',num2str(x(ad.avg.area.size.start))...
                );
            set(gh.average_stop_unit_edit,...
                'string',num2str(x(ad.avg.area.size.stop))...
                );
            set(gh.average_delta_unit_edit,...
                'string',num2str(abs(x(2)-x(1))*ad.avg.area.size.delta)...
                );
        else
            set(gh.average_start_unit_edit,...
                'string',num2str(y(ad.avg.area.size.start))...
                );
            set(gh.average_stop_unit_edit,...
                'string',num2str(y(ad.avg.area.size.stop))...
                );
            set(gh.average_delta_unit_edit,...
                'string',num2str(abs(y(2)-y(1))*ad.avg.area.size.delta)...
                );
        end
    catch exception
        try
    ...
    try
        ...
        switch ad.control.axis.displayType
            ...
            case '1D along x'
                ...
                if ad.avg.area.size.delta && strcmpi(ad.avg.dimension,'y')
                    % Plot average spectrum
                    plot(gh.axis,...
                        xvalues,...
                        mean(data(ad.avg.area.size.start:ad.avg.area.size.stop,:),1),...
                ...
                xlabel(gh.axis,sprintf('{\\it %s} / %s',xmeasure,xunit));
                if ad.avg.area.size.delta && strcmpi(ad.avg.dimension,'x')
                    ylim = get(gh.axis,'YLim');
                    patch(...
                        'XData',[...
                        xvalues(ad.avg.area.size.start) xvalues(ad.avg.area.size.start)...
                        xvalues(ad.avg.area.size.stop) xvalues(ad.avg.area.size.stop)],...
            ...
            case '1D along y'
                ...
                if ad.avg.area.size.delta && strcmpi(ad.avg.dimension,'x')
                    ...
                    plot(gh.axis,...
                        yvalues,...
                        mean(data(:,ad.avg.area.size.start:ad.avg.area.size.stop),2),...
                        'LineStyle',ad.avg.line.style,...
                        'Color',ad.avg.line.color,...
                        'LineWidth',ad.avg.line.width);
                end
                ...
                if ad.avg.area.size.delta && strcmpi(ad.avg.dimension,'y')
                    ...
                    patch(...
                        'XData',[...
                        yvalues(ad.avg.area.size.start) yvalues(ad.avg.area.size.start)...
                        yvalues(ad.avg.area.size.stop) yvalues(ad.avg.area.size.stop)],...
                        'YData',[ylim(1) ylim(end) ylim(end) ylim(1)],...
                        'ZData',[0 0 0 0],...
                        'EdgeColor',ad.avg.area.patch.edge,...
                        'FaceColor',ad.avg.area.patch.color,...
