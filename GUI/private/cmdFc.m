function [status,warnings] = cmdFc(handle,opt,varargin)
% CMDFC Command line command of the trEPR GUI.
%
% Usage:
...
% (conversion option)
if ~isnan(str2double(opt{1})) && mod(str2double(opt{1}),1) == 0
...
newFreq = str2double(opt{1});
if isnan(newFreq)
    warnings{end+1} = sprintf(...
        'Command "%s": Wrong option. Use "help %s" to get some help.',cmd,cmd);
    status = -2;
    return;
end
    
% Perform actual frequency conversion
for idx = 1:length(dataset)
    ad.data{dataset(idx)} = ...
        trEPRFC(ad.data{dataset(idx)},'frequency',newFreq);
end
