function varargout = trEPRgui_fit_helpwindow(varargin)
% TREPRGUI_fit_HELPWINDOW Brief description of GUI.
...
singleton = findobj('Tag','trEPRgui_fit_helpwindow');
if (singleton)
    figure(singleton);
    return;
end
...
%  Construct the components
hMainFigure = figure('Tag','trEPRgui_fit_helpwindow',...
    'Visible','off',...
    'Name','trEPR GUI : Fit : Help Window',...
    'Units','Pixels',...
...
uicontrol('Tag','helptopic_popupmenu',...
    ...
    'String','Introduction|Display|Fit|Settings|Report|Key bindings',...
    'KeyPressFcn',@keypress_Callback,...
...
try
    ...
    helpTextFile = fullfile(trEPRinfo('dir'),'GUI','private','helptexts','fit','intro.txt');
        ...
        switch helpText
            case 'Introduction'
                ...
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','fit','intro.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Display'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','fit','display.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Fit'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','fit','fit.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Settings'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','fit','settings.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Report'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','fit','report.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Key bindings'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','fit','keybindings.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            otherwise
