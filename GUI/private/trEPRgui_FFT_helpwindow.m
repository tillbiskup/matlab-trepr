function varargout = trEPRgui_FFT_helpwindow(varargin)
% TREPRGUI_FFT_HELPWINDOW Brief description of GUI.
...
singleton = findobj('Tag','trEPRgui_FFT_helpwindow');
if (singleton)
    figure(singleton);
    return;
end
...
%  Construct the components
hMainFigure = figure('Tag','trEPRgui_FFT_helpwindow',...
    'Visible','off',...
    'Name','trEPR GUI : FFT : Help Window',...
    'Units','Pixels',...
...
uicontrol('Tag','helptopic_popupmenu',...
    ...
    'String','Introduction|Display|Analysis|Correction|Key bindings',...
    'KeyPressFcn',@keypress_Callback,...
...
uicontrol('Tag','close_pushbutton',...
    ...
    'FontUnit','Pixel','Fontsize',12,...
    'String','Close',...
    'TooltipString','Close FFT GUI Help window',...
...
try
    ...
    helpTextFile = fullfile(trEPRinfo('dir'),'GUI','private','helptexts','FFT','intro.txt');
        ...
        switch helpText
            case 'Introduction'
                ...
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','FFT','intro.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Display'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','FFT','display.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Analysis'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','FFT','analysis.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Correction'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','FFT','correction.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Key bindings'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','FFT','keybindings.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            otherwise
