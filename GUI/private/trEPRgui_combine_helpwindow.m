...
singleton = findobj('Tag','trEPRgui_combine_helpwindow');
if (singleton)
    figure(singleton);
    return;
end
...
%  Construct the components
hMainFigure = figure('Tag','trEPRgui_combine_helpwindow',...
    'Visible','off',...
...
try
    ...
    helpTextFile = fullfile(trEPRinfo('dir'),'GUI','private','helptexts','combine','intro.txt');
        ...
        switch helpText
            case 'Introduction'
                ...
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','combine','intro.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            case 'Key bindings'
                % Read text from file and display it
                helpTextFile = fullfile(...
                    trEPRinfo('dir'),'GUI','private','helptexts','combine','keybindings.txt');
                helpText = textFileRead(helpTextFile);
                set(textdisplay,'String',helpText);
            otherwise
