...
try
    ...
    trEPRmsg('AVG GUI help window opened.','info')
    ...
    helpTextFile = fullfile(...
        trEPRinfo('dir'),'GUI','private','helptexts','AVG','intro.html');
        ...
        switch helpText
            case 'Introduction'
                ...
                helpTextFile = fullfile(trEPRinfo('dir'),...
                    'GUI','private','helptexts','AVG','intro.html');
                browser.setCurrentLocation(helpTextFile);
            case 'Display'
                % Read text from file and display it
                helpTextFile = fullfile(trEPRinfo('dir'),...
                    'GUI','private','helptexts','AVG','display.html');
                browser.setCurrentLocation(helpTextFile);
            case 'Average'
                % Read text from file and display it
                helpTextFile = fullfile(trEPRinfo('dir'),...
                    'GUI','private','helptexts','AVG','average.html');
                browser.setCurrentLocation(helpTextFile);
            case 'Settings'
                % Read text from file and display it
                helpTextFile = fullfile(trEPRinfo('dir'),...
                    'GUI','private','helptexts','AVG','settings.html');
                browser.setCurrentLocation(helpTextFile);
            case 'Key bindings'
                % Read text from file and display it
                helpTextFile = fullfile(trEPRinfo('dir'),...
                    'GUI','private','helptexts','AVG','keybindings.html');
                browser.setCurrentLocation(helpTextFile);
            otherwise
...
function closeGUI(~,~)
    try
        delete(hMainFigure);
        trEPRmsg('AVG GUI help window closed.','info');
    catch exception
        try
            msgStr = ['An exception occurred in ' ...
                exception.stack(1).name  '.'];
