...
if ~isempty(opt)
    ...
    if strfind(opt{2},':')
        % We have a range
        [avg.start,conversion1Status] = trEPRguiSanitiseNumericInput(...
            opt{2}(1:strfind(opt{2},':')-1),...
            ad.data{active}.axes.(avg.dimension).values,...
            'map',true,'index',true);
        [avg.stop,conversion2Status] = trEPRguiSanitiseNumericInput(...
            opt{2}(strfind(opt{2},':')+1:end),...
            ad.data{active}.axes.(avg.dimension).values,...
            'map',true,'index',true);
        if isnan(avg.start) || isnan(avg.stop) || conversion1Status ...
                || conversion2Status
            warnings{end+1} = sprintf(...
    ...
    elseif strfind(opt{2},';')
        ...
        [avg.start,conversion3Status] = trEPRguiSanitiseNumericInput(...
            num2str(center-width/2),...
            ad.data{active}.axes.(avg.dimension).values,...
            'map',true,'index',true);
        [avg.stop,conversion4Status] = trEPRguiSanitiseNumericInput(...
            num2str(center+width/2),...
            ad.data{active}.axes.(avg.dimension).values,...
            'map',true,'index',true);
        if isnan(avg.start) || isnan(avg.stop) || conversion1Status ...
                || conversion2Status || conversion3Status || conversion4Status
            warnings{end+1} = sprintf(...
                'Command "%s": Problems with averaging window "%s".',...
                cmd,lower(opt{2}));
            status = -3;
